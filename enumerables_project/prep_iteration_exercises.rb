...
class Array
  ...
  def bubble_sort(&prc)
    ...
    while !sorted
      sorted = true
      (1...self.length).each do |idx|
        if prc.call(sorted_arr[idx], sorted_arr[idx-1]) == -1
          sorted_arr[idx-1], sorted_arr[idx] = sorted_arr[idx], sorted_arr[idx-1]
          sorted = false
        end
      end
    end
...
# arr = [4, 5, 23, 26, 213, 0]
# arr1 = arr.bubble_sort
# p arr1
# p [6, 10, -5, 45, -89, 2].bubble_sort { |a,b| a <=> b}

# # to sort in alphabetical order
# p "kybrmh".split("").bubble_sort() do |a,b|
#   alpha = ("a".."z").to_a
...
p substrings("asdfcatqwer") # ['a', 'as', 'asd', ]
p substrings("cat") # ['a', 'as', 'asd', ]
